$(function () {
    ...
    $(".table-sort").dataTable({
        ...
        "ajax": {
            "url": "http://localhost:8080/backer/api/manager/managerList",
            "header": {
...
function admin_del(id) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            url: 'http://localhost:8080/backer/api/manager/deleteManager',
            type: 'POST', //GET
...
function admin_stop(id, status) {
    layer.confirm('确认要停用吗？', function (index) {
        ...
        $.ajax({
            url: 'http://localhost:8080/backer/api/manager/updateStatus',
            type: 'POST', //GET
            async: true,    //或false,是否异步
            headers: {
                "token": getCookie("token")
